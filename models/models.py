from sqlalchemy import Column, Integer, String, ForeignKey, Date, Point, Boolean, Float
...
class Ubication(Base):
    ...
    micro_patent = Column(String, ForeignKey('micro.patent'), nullable=False)
...
class Microbus(Base):
    ...
    linea_id = Column(String, ForeignKey('linea.number'), nullable=False)
    marca_id = Column(String, ForeignKey('marca.id'), nullable=False)
    def __repr__(self):
...
class Line (Base):
    ...
    def __repr__(self):
        return f"<Line(number={self.number}, name={self.name})>"
...
class PredictionLogVelocity(Base):
    ...
    date = Column(Date, nullable=False)
    id_micro_fk = Column(Integer, ForeignKey('micro.id'), nullable=False)
    actual = Column(Boolean, nullable=False)
